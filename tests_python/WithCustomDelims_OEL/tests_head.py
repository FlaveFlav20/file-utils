...
class TestWithCustomDelimEOFHead(unittest.TestCase):
    def test_head_n_10_valid_remove_empty_string_false(self):
        ...
        self.assertEqual(file_utils_lib.WithCustomDelims.head(path=path, n=n, delimiter=['\n']), ref)
    ...
    def test_head_n_neg_invalid_remove_empty_string_false(self):
        ...
        try:
            file_utils_lib.WithCustomDelims.head(path=path, n=n, delimiter=['\n'])
        except:
            check_pass = False
        if check_pass:
...
## 
    ...
    def test_custom_path_head_n_10_valid_remove_empty_string_false(self):
        ...
        file.close()
        ref: list = get_list(res)
        self.assertEqual(file_utils_lib.WithCustomDelims.head(path=custom_path, n=n, delimiter=['\n']), ref)
    ...
    def test_custom_path_head_n_neg_invalid_remove_empty_string_false(self):
        check_pass: bool = True 
        try:
            file_utils_lib.WithCustomDelims.head(path=path, n=n)
        except:
            check_pass = False
        if check_pass:
            self.fail("[head] Negative value shouldn't work")
...
## Test regex
    ...
    def test_head_n_10_invalid_remove_empty_string_false_keep_when_regex_valid_pass_when_regex_valid_regex_Warning(self):
        n: int = 10
        global headers
        result_to_test: list = file_utils_lib.WithCustomDelims.head(path=custom_path, n=n, delimiter=['\n'], regex_keep=["\[Warning\]:.*"], restrict=True)
