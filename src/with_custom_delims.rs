...
impl WithCustomDelims {
    ...
    pub fn head(file: String, n: usize, delimiter: Vec<String>, remove_empty_string: bool, buffer_size: usize) -> Vec<String> {
        let mut result: Vec<String> = Vec::new();
        let mut read: ReadDelimiter = ReadDelimiter::new(file, delimiter, buffer_size).expect("Unable to initialize delimiter");
    ...
    pub fn between(file: String, n1: usize, n2: usize, delimiter: Vec<String>, remove_empty_string: bool, buffer_size: usize) -> Vec<String> {
        let mut result: Vec<String> = Vec::new();
        let mut read: ReadDelimiter = ReadDelimiter::new(file, delimiter, buffer_size).expect("Unable to initialize delimiter");
    ...
    #[staticmethod]
    #[pyo3(signature = (file, n, delimiter, remove_empty_string=false, buffer_size = 1024))]
    pub fn tail(file: String, n: usize, delimiter: Vec<String>, remove_empty_string: bool, buffer_size: usize) -> Vec<String> {
    ...
    pub fn count_lines(file: String, delimiter: Vec<String>, remove_empty_string: bool, buffer_size: usize) -> usize {
        let mut res: usize = 0;
        let mut read: ReadDelimiter = ReadDelimiter::new(file, delimiter, buffer_size).expect("Unable to initialize delimiter");
        while read.read().expect("Unable to read delimiter") == true {
            if remove_empty_string && read.line.to_string().is_empty() {
